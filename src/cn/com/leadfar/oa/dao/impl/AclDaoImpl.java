...
public class AclDaoImpl extends BaseDaoImpl implements AclDao {
	...
	public void delAcls(String principalType, int principalId,
		...
		//首先找出所有授权记录
		Iterator acls = getSession().createQuery("select a from ACL a where a.principalType = ? " +
	...
	public ACL findACL(String principalType, int principalId,
		...
		return (ACL)getSession().createQuery("select a from ACL a where a.principalType = ?" +
	...
	public List<ACL> findAcls(String principalType, int principalId,
		...
		return getSession().createQuery("select a from ACL a where a.principalType = ? " +
			"and a.principalId = ? and a.resourceType = ?")
			.setParameter(0, principalType)
			.setParameter(1, principalId)
	...
	public SysResource findSysResourceBySn(String resourceSn) {
		String hql = "select sr from cn.com.leadfar.oa.model.SysResource sr where sr.sn=?";
